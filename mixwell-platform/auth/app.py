...
from flask import Flask, make_response, redirect, request, jsonify, render_template, flash, session, url_for
import requests
...
# -------------------------
# user signup, login, logout request from potal 
# -------------------------
...
@app.route("/user/user_signup", methods=["GET"])
def user_signup():    
...
def user_login():    
    ...
    email = request.form["username"]
    password = request.form["password"]    
    respose = Utility.user_login(email, password)


@app.route("/user/user_reset/<int:userid>")  
def user_reset(userid):
    user = request.get_data()
    Utility.user_reset(userid)    
...
@app.route("/user/user_without_services/<int:userid>", methods=["GET"])
def user_without_services(userid):    
    return Utility.user_without_services(userid)

@app.route("/user/user_with_services/<int:userid>", methods=["GET"])
...
@app.route("/user/user_get/<int:userid>", methods=["GET"]) #
def user_get(userid):         
    user = Utility.user_get(userid)
    return user
